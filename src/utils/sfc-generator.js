...
const elTemplates = {  //字段组件属性
  ...
  'picture-upload': (widget, formConfig) => {
    const {vModel, disabled, uploadAction, uploadName, withCredentials, multipleSelect, showFileList, limit,
    ...
    return `<el-upload :file-list="${wop.name}FileList" :headers="${wop.name}UploadHeaders" :data="${wop.name}UploadData" 
            ${disabled} ${uploadAction} ${uploadName} list-type="picture-card" ${withCredentials} ${multipleSelect} ${showFileList} 
