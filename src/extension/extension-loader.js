...
import {readOnlySchema} from "@/extension/samples/extension-schema"
import ReadOnlyWidget from '@/extension/samples/readOnly/read-only-widget'
import {readOnlyTemplateGenerator} from '@/extension/samples/extension-sfc-generator'
...
import { hrefSchema } from "@/extension/samples/extension-schema"
import HrefWidget from '@/extension/samples/href/href.vue'
import {hrefGenerator} from '@/extension/samples/extension-sfc-generator'
...
export const loadExtension = function () {
    ...
    // loadReadOnlyWidget()
    loadSelectGroupWidget()
    loadHrefpWidget()
}
const loadHrefpWidget = () => {
    /**
   * 加载字段组件步骤：
   * 1. 加载组件Json Schema;
   ...
   * 5. 加载完毕。
   */
    addCustomWidgetSchema(hrefSchema)  //加载组件Json Schema
    Vue.component(HrefWidget.name, HrefWidget)  //注册组件
    registerFWGenerator('href', hrefGenerator)  //注册字段组件的代码生成器
}
const loadSelectGroupWidget = () => {
    /**
   * 加载字段组件步骤：
   * 1. 加载组件Json Schema;
   * 2. 全局注册字段组件，字段组件设计期和运行期共用，故需要仅需注册一个组件；
   ...
   */
    ...
    Vue.component(SelectGroupWidget.name, SelectGroupWidget)  //注册组件
    registerFWGenerator('selectGroup', selectGroupGenerator)  //注册字段组件的代码生成器
}
const loadReadOnlyWidget = () => {
    /**
   * 加载字段组件步骤：
   * 1. 加载组件Json Schema;
   * 2. 全局注册字段组件，字段组件设计期和运行期共用，故需要仅需注册一个组件；
   ...
   * 5. 加载完毕。
   */
    addCustomWidgetSchema(readOnlySchema)  //加载组件Json Schema
    Vue.component(ReadOnlyWidget.name, ReadOnlyWidget)  //注册组件
    registerFWGenerator('readOnly', readOnlyTemplateGenerator)  //注册字段组件的代码生成器
}
const loadAutoCompleteWidget = () => {
    /**
   * 加载字段组件步骤：
   * 1. 加载组件Json Schema;
   * 2. 全局注册字段组件，字段组件设计期和运行期共用，故需要仅需注册一个组件；
   ...
   */
     ...
     Vue.component(AutoCompleteWidget.name, AutoCompleteWidget)  //注册组件
