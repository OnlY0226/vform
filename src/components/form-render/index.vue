  ...
  <el-form :label-position="labelPosition" :size="size" :class="[customClass]" class="render-form"
    ...
    <template v-for="(widget, index) in widgetList">
      <template v-if="'container' === widget.category">
        <component :is="getContainerWidgetName(widget)" :widget="widget" :key="widget.id" :designer="null" :parent-list="widgetList"
                        :index-of-parent-list="index" :parent-widget="null" @handlePreview="handlePreview">
          <!-- 递归传递插槽！！！ -->
  ...
  export default {
    ...
    methods: {
      ...
      getFormData(needValidation = true) {
        ...
        this.$refs['renderForm'].validate((valid, data) => {
          if (valid) {
            callback(this.formDataModel)
          } else {
            callback(this.formDataModel, { msg:this.i18nt('render.hint.validationFailed'), data })
          }
        })
