...
<template>
  <div class="field-wrapper" :class="{'design-time-bottom-margin': !!this.designer}" :style="`--label-width:${labelWidth}px;`">
    <el-form-item v-if="!!field.formItemFlag" :label="label" :label-width="labelWidth + 'px'"
                  ...
                  @click.native.stop="selectField(field)" ref="elFormItemRef">
      ...
      <span v-if="!!field.options.labelIconClass" slot="label" class="custom-label">
        <template v-if="field.options.labelIconPosition === 'front'">
          <template v-if="!!field.options.labelTooltip">
            <el-tooltip effect="dark">
              <div v-html="field.options.labelTooltip" slot="content"></div>
              <i :class="field.options.labelIconClass"></i>
            </el-tooltip>{{label}}
          </template>
          <template v-else>
            <i :class="field.options.labelIconClass"></i>{{label}}
          </template>
        </template>
        <template v-else-if="field.options.labelIconPosition === 'rear'">
          <template v-if="!!field.options.labelTooltip">
            ...
            <el-tooltip effect="dark">
              <div v-html="field.options.labelTooltip" slot="content"></div>
              <i :class="[`${field.options.labelIconClass}`, {'labelIcon--rear': labelAlign === 'label-right-align'}]"></i>
            </el-tooltip>
          </template>
          <template v-else>
            {{label}}
  ...
  export default {
    ...
    computed: {
      ...
      label() {
        ...
        const lang = localStorage.getItem("v_form_locale")
        // TODO change language by user setting
        // and get label from sever
        return lang === 'zh-TW' ?  this.field.options.labelZh : this.field.options.label
      },
    ...
    methods: {
      ...
      validate () {
        this.$refs.elFormItemRef.validate()
      }
    }
  }
</script>
