<template>
  <form-item-wrapper :designer="designer" :field="field" :rules="rules" :design-state="designState"
                     ...
                     :class="{'dp-is-disabled': field.options.disabled, 'upload--drag': field.options.drag}" ref="uploadRef">
    ...
    <el-upload ref="fieldEditor" :disabled="field.options.disabled"
               ...
               :action="field.options.uploadURL" :headers="uploadHeaders" :data="uploadData" name="files"
               :drag="field.options.drag"
      ...
      <template v-if="field.options.drag">
        <i class="el-icon-upload"></i>
        <div class="el-upload__text">{{i18nt('designer.widgetLabel.dragTip1')}}<em>{{i18nt('designer.widgetLabel.dragTip2')}}</em></div>
      </template>
      <template v-else>
  ...
  export default {
    ...
    methods: {
      ...
      removeUploadFile(fileName) {
        ...
        console.log(this.field.options.required);
        console.log(this.fileList.length);
        console.log(this.field.options.required && this.fileList.length === 0);
        if (this.field.options.required && this.fileList.length === 0 ) {
          console.log('???????????');
           this.$refs.uploadRef.validate()
        }
      },
      ...
      handlePreview(file) {
        //console.log(file, 'file-upload-widget');
...
<style lang="scss" scoped>
  ...
  .dp-is-disabled ::v-deep .el-upload.el-upload--text {
    display: none;
  }
  .upload--drag ::v-deep .el-upload {
    width: 100%;
    &-dragger {
      width: 100%;
    }
