...
export const advancedFields = [
...

  {
    ...
    options: {
      ...
      onCreated: '',
      onMounted: '',
      onBeforeUpload: "// this.setOption('limit', 10)\n// this.setOption('uploadTip', 'test') \n //  this.setOption('fileMaxSize', 4)",
      onUploadSuccess: '',
      onUploadError: '',
      onFileRemove: "const id = file.id || file.response[0].contentId\nif (!id) return\nthis.$axios.delete(`/docpal/workflow/task/attachment?attachmentId=${id}`).then(res => {\n  this.getFormRef().$emit('handleSave')\n}).catch(err => {\n})",
      onValidate: '',
