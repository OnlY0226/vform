        ...
        <template v-for="(ft, idx) in formTemplates">
          <el-card :key="idx" :bord-style="{ padding: '0' }" shadow="hover" class="ft-card">
            <el-popover placement="right" trigger="hover">
              <template v-if="ft.imgUrl">
                <img slot="reference" :src="ft.imgUrl" style="width: 200px">
                <img :src="ft.imgUrl" style="width: 720px">
              </template>
              <template v-else-if="ft.imgDevPath">
                <img slot="reference" :src="require(`${ft.imgDevPath}`)" style="width: 200px">
                <img :src="require(`${ft.imgDevPath}`)" style="width: 720px">
              </template>
            </el-popover>
            <div class="bottom clear-fix">
              <span class="ft-title">#{{idx+1}} {{ft.title}}</span>
              <el-button type="text" class="right-button" @click="loadFormTemplate(ft.jsonUrl, ft)">
...
<script>
  ...
  import workflowBase from "./templates/workflowBase.json"
  ...
  export default {
    ...
    data() {
      return {
        ...
        //   {imgUrl: ftImg8},
        // ]
        src: './templates/templatesPhoto/workfolwBase.png'
      }
    },
    computed: {
      //
    ...
    methods: {
        ...
        this.$confirm(this.i18nt('designer.hint.loadFormTemplateHint'), this.i18nt('render.hint.prompt'), {
          ...
          if (jsonUrl) {
            axios.get(jsonUrl).then(res => {
              this.handleLoadFormTemplateData(res.data)
            }).catch(error => {
              this.$message.error(this.i18nt('designer.hint.loadFormTemplateFailed') + ':' + error)
            })
          } else {
            try {
              const json = require(`${ft.jsonDevPath}`);
              this.handleLoadFormTemplateData(json)
            } catch {
              this.$message.error(this.i18nt('designer.hint.loadFormTemplateFailed') )
            }
          }
        }).catch(error => {
          console.error(error)
      ...
      handleLoadFormTemplateData(json) {
