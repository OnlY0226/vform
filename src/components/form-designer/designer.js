...
export function createDesigner(vueInstance) {
  let defaultFormConfig = {
    ...
    saveRemoteOptions: 'never', // never | ask | always
    cssCode: '',
    customClass: '',
    functions: '',
    layoutType: 'PC',
  ...
  return {
    ...
    initDesigner(resetFormJson) {
      ...
      console.info(`%cVariantForm %cVer${VARIANT_FORM_VERSION} %chttps://www.yuque.com/variantdev/vform`,
          "color:#17B2C0;font-size: 22px;font-weight:bolder",
    ...
    updateSelectedWidgetNameAndRef(selectedWidget, newName, newLabel, labelZh) {
      ...
      if (!!newLabel && (Object.keys(selectedWidget.options).indexOf('label') > -1)) {
        ...
        selectedWidget.options.labelZh = labelZh
      }
    },
    ...
    copyNewFieldWidget(origin) {
      ...
      newWidget.options.labelZh = newWidget.options.labelZh || newWidget.type.toLowerCase()
    ...
    copyNewContainerWidget(origin) {
      ...
      if (newCon.type === 'grid') {
        ...
        newTabPane.options.labelZh = '标签 1'
    ...
    addTabPaneOfTabs(tabsWidget) {
      ...
      newTabPane.options.labelZh = '标签 ' + (tabPanes.length + 1)
