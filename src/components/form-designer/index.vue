...
<template>
  <el-container class="main-container full-height v-form-designer">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // },
      loadFieldListFromServer() {
        if (!this.fieldListApi && !this.fieldListApi.fieldData) {
          return
        }
        this.handleLoadFieldList(this.fieldListApi.fieldData)
      },
      handleLoadFieldList(list) {
        let labelKey = this.fieldListApi.labelKey || 'label'
        let labelZh = this.fieldListApi.labelZh || 'labelZh'
        let nameKey = this.fieldListApi.nameKey || 'name'
        this.fieldList.splice(0, this.fieldList.length)  //清空已有
        list.forEach(fieldItem => {
          this.fieldList.push({
            label: fieldItem[labelKey],
            labelZh: fieldItem[labelZh],
            name: fieldItem[nameKey]
          })
        })
      },
      ...
      handleRemoteOptions () {
        if (!this.designer.formConfig || this.designer.formConfig.saveRemoteOptions === 'always') return
        const result = {}
        const widgetList = this.getFieldWidgets()
        const typeList = ['auto-complete', 'select', 'radio', 'checkbox']
        widgetList.forEach(widgetItem => {
          if (typeList.indexOf(widgetItem.type) > -1 ) {
            this.funList.forEach(funItem => {
              if (widgetItem.field.options[funItem] && widgetItem.field.options[funItem].indexOf('loadOptions') > 0) widgetItem.field.options.optionItems = []
            })
          }
        })
