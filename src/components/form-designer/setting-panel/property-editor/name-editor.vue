  ...
  <el-form-item prop="name" :rules="nameRequiredRule">
    ...
    <template v-else>
      <el-select v-model="optionModel.name" allow-create filterable :disabled="widgetNameReadonly" @change="updateWidgetNameAndRef"
        ...
        <el-option v-for="(sf, sfIdx) in serverFieldList" :key="sfIdx" :label="sf.name" :value="sf.name"></el-option>
      </el-select>
    </template>
  </el-form-item>
</template>
      ...
      updateWidgetNameAndRef(newName) {
        ...
        if (!!this.designer.formWidget) {
          ...
          if (!!fieldWidget && !!fieldWidget.registerToRefList) {
            ...
            let labelEn = this.getLabelByFieldName(newName)
            let labelZh = this.getLabelByFieldName(newName, 'zh')
            this.designer.updateSelectedWidgetNameAndRef(this.selectedWidget, newName, labelEn, labelZh)
          }
        }
      },

      getLabelByFieldName(fieldName, type='en') {
        for (let i = 0; i < this.serverFieldList.length; i++) {
          if (this.serverFieldList[i].name === fieldName) {
            return type ==='en' ? this.serverFieldList[i].label : this.serverFieldList[i].labelZh
          }
        }
